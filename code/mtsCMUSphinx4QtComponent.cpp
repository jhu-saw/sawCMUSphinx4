...
CMN_IMPLEMENT_SERVICES(mtsCMUSphinx4QtComponent);


mtsCMUSphinx4QtComponent::mtsCMUSphinx4QtComponent(const std::string & componentName):
    mtsComponent(componentName)
{
    // create the cisstMultiTask interface with commands and events
    mtsInterfaceRequired * interfaceRequired = AddInterfaceRequired("CMUSphinx4");
    if (interfaceRequired) {
        ...
        interfaceRequired->AddEventHandlerWrite(&mtsCMUSphinx4QtComponent::WordRecognizedHandler,
                                                this, "WordRecognized");
        interfaceRequired->AddEventHandlerVoid(&mtsCMUSphinx4QtComponent::NoWordRecognizedHandler,
                                               this, "NoWordRecognized");
        interfaceRequired->AddEventHandlerWrite(&mtsCMUSphinx4QtComponent::ContextChangedHandler,
                                                this, "ContextChanged");
    }
...
void mtsCMUSphinx4QtComponent::Start(void)
{
...
QWidget * mtsCMUSphinx4QtComponent::GetWidget(void)
{
    return &CentralWidget;
}


void mtsCMUSphinx4QtComponent::TriggerWord(QString word)
{
    TriggerWordFromUI(mtsStdString(word.toStdString()));
}


void mtsCMUSphinx4QtComponent::WordRecognizedHandler(const mtsStdString & word)
{
    emit WordRecognizedQSignal(QString(word.Data.c_str()));
}


void mtsCMUSphinx4QtComponent::NoWordRecognizedHandler(void)
{
    emit NoWordRecognizedQSignal();
}


void mtsCMUSphinx4QtComponent::ContextChangedHandler(const mtsStdString & context)
{
